...
def main():
    ...
    try:
        ...
        if result["status"] == "success":
            ...
            print(
                f"   - Eventos procesados: {meta.get('rows_after_validation', 'N/A')}"
            )
            print(f"   - Duplicados eliminados: {meta.get('duplicates_removed', 0)}")
            print(f"   - Ruta Parquet: {store['s3_path']}")
            print(
                f"   - Tamaño original (JSON): ~{os.path.getsize(bronze_key) if os.path.exists(bronze_key) else 'N/A'} bytes (estimado)"
            )
            ...
            df_check = pd.read_parquet(
                store["s3_path"]
                .replace("s3://", f"{etl.storage.endpoint_url}/")
                .replace(
                    etl.storage.endpoint_url + "/" + etl.bucket_silver,
                    f"s3://{etl.bucket_silver}",
                ),
                storage_options={
            ...
            print(
                f"   ✅ Leído correctamente con Pandas. Columnas: {list(df_check.columns)}"
            )
