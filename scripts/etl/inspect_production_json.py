...
def main():
    ...
    for raid_dir in raid_dirs:
        ...
        for etype, count in sorted(stats["type_counter"].items(), key=lambda x: -x[1]):
    ...
    for role, count in sorted(global_roles.items(), key=lambda x: -x[1]):
        print(
            f"   {role:<12} {count:>8,}  {format_pct(count, sum(global_roles.values()))}"
        )

    print("\n  Top 10 clases (source_player_class):")
    for cls, count in global_classes.most_common(10):
        print(
            f"   {cls:<15} {count:>8,}  {format_pct(count, sum(global_classes.values()))}"
        )
    ...
    checks = [
        ...
        (global_events >= 490_000, f"~500k eventos totales        → {global_events:,}"),
        ("combat_damage" in global_types, "Tipo 'combat_damage' presente"),
        (
