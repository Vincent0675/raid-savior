...
def inspect_bronze_file(bronze_key):
    ...
    print(
        df_bronze[
            ["event_id", "event_type", "timestamp", "damage_amount", "healing_amount"]
        ]
        .head(2)
        .to_string()
    )
...
def inspect_silver_file(batch_id, raid_id):
    ...
    try:
        ...
        print(
            "   - Columnas nuevas (enriquecimiento): is_massive_hit, ingest_latency_ms"
        )
...
def compare_layers(bronze_data, df_bronze, df_silver, bronze_size, silver_size):
    ...
    print(
        f"      - timestamp: {df_silver['timestamp'].dtype} (datetime con nanosegundos)"
    )
    if "damage_amount" in df_silver.columns:
        print(
            f"      - damage_amount: {df_silver['damage_amount'].dtype} (float nativo)"
        )
    if "is_critical_hit" in df_silver.columns:
        print(
            f"      - is_critical_hit: {df_silver['is_critical_hit'].dtype} (booleano)"
        )
    ...
    print(
        "   Ventaja: Las columnas raid_id y event_date ya NO están dentro del Parquet"
    )
...
def main():
    ...
    print(
        "   Los datos pasaron de JSON crudo a Parquet optimizado, limpio y enriquecido.\n"
    )
