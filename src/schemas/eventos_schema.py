...
class WoWRaidEvent(BaseModel):
    ...
    # ====== IDENTIFICACIÓN (Core) ======
    event_id: UUID = Field(
        default_factory=uuid4, description="Unique event identifier (UUID v4)"
    )
    ...
    encounter_id: str | None = Field(
        default=None, description="Boss encounter identifier (optional)"
    )

    encounter_duration_ms: int | None = Field(
        default=None, ge=0, description="Duration of encounter in milliseconds"
    )

    # ====== ACTOR (Source Player) ======
    source_player_id: str = Field(min_length=1, description="Unique player identifier")

    source_player_name: str = Field(
        min_length=1, max_length=12, description="Player name"
    )

    source_player_role: PlayerRole | None = Field(
        default=None, description="Player role (tank, healer, dps)"
    )

    source_player_class: PlayerClass | None = Field(
        default=None, description="Player class (warrior, mage, etc.)"
    )

    source_player_level: int | None = Field(
        default=None, ge=1, le=90, description="Player level (1-90)"
    )

    # ====== ACCIÓN (Ability) ======
    ability_id: str | None = Field(default=None, description="Ability/spell identifier")

    ability_name: str | None = Field(default=None, description="Ability/spell name")

    ability_school: DamageSchool | None = Field(
        default=None, description="Damage/healing school (fire, frost, holy, etc.)"
    )
    ...
    healing_amount: float | None = Field(
        default=None, ge=0, le=500_000, description="Healing done (for heal events)"
    )

    is_critical_hit: bool = Field(
        default=False, description="Was this a critical hit/heal?"
    )
    ...
    target_entity_name: str | None = Field(
        default=None, description="Target entity name"
    )

    target_entity_type: EntityType | None = Field(
        default=None, description="Type of target (player, boss, add, interactive)"
    )

    target_entity_health_pct_before: float | None = Field(
        default=None, ge=0, le=100, description="Target health percentage before event"
    )

    target_entity_health_pct_after: float | None = Field(
        default=None, ge=0, le=100, description="Target health percentage after event"
    )

    # ====== RECURSOS (Para mana_regeneration) ======
    resource_type: ResourceType | None = Field(
        default=None, description="Type of resource (mana, energy, rage, etc.)"
    )

    resource_amount_before: float | None = Field(
        default=None, ge=0, description="Resource amount before event"
    )

    resource_amount_after: float | None = Field(
        default=None, ge=0, description="Resource amount after event"
    )

    resource_regeneration_rate: float | None = Field(
        default=None, ge=0, description="Resource regeneration rate (units per second)"
    )
    ...
    source_system: str = Field(
        default="wow-raid-addon-v1.2", description="System that generated this event"
    )
    ...
    server_latency_ms: int | None = Field(
        default=None, ge=0, description="Server latency in milliseconds"
    )

    client_latency_ms: int | None = Field(
        default=None, ge=0, description="Client latency in milliseconds"
    )
    ...
    @classmethod
    def validate_resource_for_mana_events(
        cls, v: ResourceType | None, info
    ) -> ResourceType | None:
        """
