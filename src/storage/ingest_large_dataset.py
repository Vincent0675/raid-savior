...
class DatasetIngestor:
    ...
    def ingest_via_http(
        self, events: list[dict[str, Any]], batch_size: int = 1000, max_retries: int = 3
    ):
        """
                    ...
                    try:
                        ...
                        if response.status_code == 201:
                            ...
                            if attempt == max_retries - 1:
                                failed += len(batch)
                                tqdm.write(
                                    f"❌ Batch {i // batch_size + 1} falló: {response.status_code}"
                                )
                        ...
                        if attempt == max_retries - 1:
                            failed += len(batch)
                            tqdm.write(
                                f"❌ Batch {i // batch_size + 1} error de red: {e}"
                            )
                        else:
    ...
    def ingest_via_s3(self, events: list[dict[str, Any]], raid_id: str = "raid001"):
        """
        ...
        total_written = 0
        for ingest_date, date_events in tqdm(
            events_by_date.items(), desc="Escribiendo particiones"
        ):
    ...
    def run(self, num_events: int, batch_size: int = 1000):
        ...
        session = gen.generate_raid_session(
            raid_id="raid001", num_players=25, duration_s=480, boss_name="Lich King"
        )
...
def main():
    ...
    parser.add_argument(
        "--batch-size", type=int, default=1000, help="Tamaño de batch para modo HTTP"
    )
